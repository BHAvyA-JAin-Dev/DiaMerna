/* DiaMerna — Persistent Backend Server
   Single source of truth for web, desktop, and mobile.
   All platforms connect here for data.
   Deploy on Render / Railway / Fly.io with persistent volume. */
...
const JWT_SECRET = process.env.JWT_SECRET || 'diamerna_persistent_jwt_' + crypto.randomUUID()
const uuid = () => crypto.randomUUID()
const WEBAPP_PATH = process.env.WEBAPP_PATH || path.join(__dirname, '..', 'dev')

/* CORS — allow all origins (desktop, mobile, web) */
app.use(cors())
...
function initDB() {
  ...
  db.exec(`
    ...
    /* Sync tables — user health data shared across platforms */
    CREATE TABLE IF NOT EXISTS weight_logs (
      id TEXT PRIMARY KEY, user_id TEXT NOT NULL, weight REAL NOT NULL,
    ...
    CREATE TABLE IF NOT EXISTS contraction_logs (
      id TEXT PRIMARY KEY, user_id TEXT NOT NULL,
      start_time TEXT NOT NULL, end_time TEXT, duration_sec INTEGER,
      date TEXT NOT NULL, created_at TEXT DEFAULT (datetime('now')),
      FOREIGN KEY(user_id) REFERENCES users(id)
    );
    CREATE TABLE IF NOT EXISTS glucose_logs (
      id TEXT PRIMARY KEY, user_id TEXT NOT NULL, value REAL NOT NULL,
      meal_type TEXT, notes TEXT, date TEXT NOT NULL,
      created_at TEXT DEFAULT (datetime('now')),
      FOREIGN KEY(user_id) REFERENCES users(id)
    );
    CREATE TABLE IF NOT EXISTS baby_growth_logs (
      id TEXT PRIMARY KEY, user_id TEXT NOT NULL,
      week INTEGER NOT NULL, weight REAL, height REAL,
      notes TEXT, date TEXT NOT NULL,
      created_at TEXT DEFAULT (datetime('now')),
      FOREIGN KEY(user_id) REFERENCES users(id)
    );
...
/* ===== Dropbox helpers (same as api/index.js) ===== */
...
async function dropboxValidToken(userId, provider) {
...
app.post('/api/login', async (req, res) => {
  ...
  if (rl.count > 10) return res.status(429).json({ error: 'Too many attempts. Try again in 15 minutes.' })
...
app.get('/api/oauth/callback', async (req, res) => {
  ...
  const creds = OAUTH.dropbox
  if (!creds || !creds.client_id) { res.status(400).send('<h2>Dropbox OAuth not configured on server.</h2>'); return }
  try {
    ...
    res.redirect('/?oauth_success=dropbox&token=' + encodeURIComponent(newToken))
...
app.post('/api/cloud/upload', async (req, res) => {
  ...
  if (provider !== 'dropbox') return res.status(400).json({ error: 'Only Dropbox is supported' })
  let tok = await dropboxValidToken(u.id, provider)
...
/* ===== SYNC ENDPOINTS — Cross-platform data sync ===== */

/* Weight Logs */
app.post('/api/sync/weight', (req, res) => {
  const u = auth(req); if (!u) return res.status(401).json({ error: 'Unauthorized' })
  const { logs } = req.body
  if (!Array.isArray(logs)) return res.status(400).json({ error: 'logs array required' })
  const ins = db.prepare('INSERT OR REPLACE INTO weight_logs (id, user_id, weight, date) VALUES (?, ?, ?, ?)')
  const tx = db.transaction(() => { for (const l of logs) ins.run(l.id || uuid(), u.id, l.weight, l.date || new Date().toISOString().split('T')[0]) })
  tx()
  res.json({ synced: logs.length })
})

app.get('/api/sync/weight', (req, res) => {
  const u = auth(req); if (!u) return res.status(401).json({ error: 'Unauthorized' })
  const rows = db.prepare('SELECT * FROM weight_logs WHERE user_id = ? ORDER BY date DESC LIMIT 100').all(u.id)
  res.json({ logs: rows })
})

/* Blood Pressure */
app.post('/api/sync/bp', (req, res) => {
  const u = auth(req); if (!u) return res.status(401).json({ error: 'Unauthorized' })
  const { logs } = req.body
  if (!Array.isArray(logs)) return res.status(400).json({ error: 'logs array required' })
  const ins = db.prepare('INSERT OR REPLACE INTO bp_logs (id, user_id, systolic, diastolic, date) VALUES (?, ?, ?, ?, ?)')
  const tx = db.transaction(() => { for (const l of logs) ins.run(l.id || uuid(), u.id, l.systolic, l.diastolic, l.date || new Date().toISOString().split('T')[0]) })
  tx()
  res.json({ synced: logs.length })
})

app.get('/api/sync/bp', (req, res) => {
  const u = auth(req); if (!u) return res.status(401).json({ error: 'Unauthorized' })
  const rows = db.prepare('SELECT * FROM bp_logs WHERE user_id = ? ORDER BY date DESC LIMIT 100').all(u.id)
  res.json({ logs: rows })
})

/* Contraction Logs */
app.post('/api/sync/contractions', (req, res) => {
  const u = auth(req); if (!u) return res.status(401).json({ error: 'Unauthorized' })
  const { logs } = req.body
  if (!Array.isArray(logs)) return res.status(400).json({ error: 'logs array required' })
  const ins = db.prepare('INSERT OR REPLACE INTO contraction_logs (id, user_id, start_time, end_time, duration_sec, date) VALUES (?, ?, ?, ?, ?, ?)')
  const tx = db.transaction(() => { for (const l of logs) ins.run(l.id || uuid(), u.id, l.start_time, l.end_time, l.duration_sec, l.date || new Date().toISOString().split('T')[0]) })
  tx()
  res.json({ synced: logs.length })
})

app.get('/api/sync/contractions', (req, res) => {
  const u = auth(req); if (!u) return res.status(401).json({ error: 'Unauthorized' })
  const rows = db.prepare('SELECT * FROM contraction_logs WHERE user_id = ? ORDER BY start_time DESC LIMIT 100').all(u.id)
  res.json({ logs: rows })
})

/* Glucose Logs */
app.post('/api/sync/glucose', (req, res) => {
  const u = auth(req); if (!u) return res.status(401).json({ error: 'Unauthorized' })
  const { logs } = req.body
  if (!Array.isArray(logs)) return res.status(400).json({ error: 'logs array required' })
  const ins = db.prepare('INSERT OR REPLACE INTO glucose_logs (id, user_id, value, meal_type, notes, date) VALUES (?, ?, ?, ?, ?, ?)')
  const tx = db.transaction(() => { for (const l of logs) ins.run(l.id || uuid(), u.id, l.value, l.meal_type || '', l.notes || '', l.date || new Date().toISOString().split('T')[0]) })
  tx()
  res.json({ synced: logs.length })
})

app.get('/api/sync/glucose', (req, res) => {
  const u = auth(req); if (!u) return res.status(401).json({ error: 'Unauthorized' })
  const rows = db.prepare('SELECT * FROM glucose_logs WHERE user_id = ? ORDER BY date DESC LIMIT 100').all(u.id)
  res.json({ logs: rows })
})

/* Baby Growth Logs */
app.post('/api/sync/baby-growth', (req, res) => {
  const u = auth(req); if (!u) return res.status(401).json({ error: 'Unauthorized' })
  const { logs } = req.body
  if (!Array.isArray(logs)) return res.status(400).json({ error: 'logs array required' })
  const ins = db.prepare('INSERT OR REPLACE INTO baby_growth_logs (id, user_id, week, weight, height, notes, date) VALUES (?, ?, ?, ?, ?, ?, ?)')
  const tx = db.transaction(() => { for (const l of logs) ins.run(l.id || uuid(), u.id, l.week, l.weight, l.height, l.notes || '', l.date || new Date().toISOString().split('T')[0]) })
  tx()
  res.json({ synced: logs.length })
})

app.get('/api/sync/baby-growth', (req, res) => {
  const u = auth(req); if (!u) return res.status(401).json({ error: 'Unauthorized' })
  const rows = db.prepare('SELECT * FROM baby_growth_logs WHERE user_id = ? ORDER BY week DESC LIMIT 100').all(u.id)
  res.json({ logs: rows })
})

/* Full sync — push all local data to server in one call */
app.post('/api/sync/push-all', (req, res) => {
  const u = auth(req); if (!u) return res.status(401).json({ error: 'Unauthorized' })
  const { weight, bp, contractions, glucose, babyGrowth } = req.body
  const results = {}
  try {
    if (Array.isArray(weight)) {
      const ins = db.prepare('INSERT OR REPLACE INTO weight_logs (id, user_id, weight, date) VALUES (?, ?, ?, ?)')
      db.transaction(() => { for (const l of weight) ins.run(l.id || uuid(), u.id, l.weight, l.date || new Date().toISOString().split('T')[0]) })()
      results.weight = weight.length
    }
    if (Array.isArray(bp)) {
      const ins = db.prepare('INSERT OR REPLACE INTO bp_logs (id, user_id, systolic, diastolic, date) VALUES (?, ?, ?, ?, ?)')
      db.transaction(() => { for (const l of bp) ins.run(l.id || uuid(), u.id, l.systolic, l.diastolic, l.date || new Date().toISOString().split('T')[0]) })()
      results.bp = bp.length
    }
    if (Array.isArray(contractions)) {
      const ins = db.prepare('INSERT OR REPLACE INTO contraction_logs (id, user_id, start_time, end_time, duration_sec, date) VALUES (?, ?, ?, ?, ?, ?)')
      db.transaction(() => { for (const l of contractions) ins.run(l.id || uuid(), u.id, l.start_time, l.end_time, l.duration_sec, l.date || new Date().toISOString().split('T')[0]) })()
      results.contractions = contractions.length
    }
    if (Array.isArray(glucose)) {
      const ins = db.prepare('INSERT OR REPLACE INTO glucose_logs (id, user_id, value, meal_type, notes, date) VALUES (?, ?, ?, ?, ?, ?)')
      db.transaction(() => { for (const l of glucose) ins.run(l.id || uuid(), u.id, l.value, l.meal_type || '', l.notes || '', l.date || new Date().toISOString().split('T')[0]) })()
      results.glucose = glucose.length
    }
    if (Array.isArray(babyGrowth)) {
      const ins = db.prepare('INSERT OR REPLACE INTO baby_growth_logs (id, user_id, week, weight, height, notes, date) VALUES (?, ?, ?, ?, ?, ?, ?)')
      db.transaction(() => { for (const l of babyGrowth) ins.run(l.id || uuid(), u.id, l.week, l.weight, l.height, l.notes || '', l.date || new Date().toISOString().split('T')[0]) })()
      results.babyGrowth = babyGrowth.length
    }
    res.json({ synced: results })
  } catch (e) { res.status(500).json({ error: 'Sync failed', detail: e.message }) }
})

/* Pull all — get all user data from server */
app.get('/api/sync/pull-all', (req, res) => {
  const u = auth(req); if (!u) return res.status(401).json({ error: 'Unauthorized' })
  const data = {
    weight: db.prepare('SELECT * FROM weight_logs WHERE user_id = ? ORDER BY date ASC').all(u.id),
    bp: db.prepare('SELECT * FROM bp_logs WHERE user_id = ? ORDER BY date ASC').all(u.id),
    contractions: db.prepare('SELECT * FROM contraction_logs WHERE user_id = ? ORDER BY start_time ASC').all(u.id),
    glucose: db.prepare('SELECT * FROM glucose_logs WHERE user_id = ? ORDER BY date ASC').all(u.id),
    babyGrowth: db.prepare('SELECT * FROM baby_growth_logs WHERE user_id = ? ORDER BY week ASC').all(u.id)
  }
  res.json(data)
})
...
app.get('/api/admin/stats', (req, res) => {
  const h = req.headers['authorization'] || ''
  const t = h.replace('Bearer ', '')
  try { const p = jwt.verify(t, JWT_SECRET); if (p.role !== 'admin') throw new Error('Not admin') } catch { return res.status(401).json({ error: 'Unauthorized' }) }
...
app.post('/api/admin/settings', (req, res) => {
  ...
  const tx = db.transaction(() => { for (const [key, value] of Object.entries(settings)) upsert.run(key, String(value)) })
  tx()
...
app.get('/api/health', (req, res) => {
  res.json({ status: 'ok', uptime: process.uptime(), db: DB_PATH, version: '1.0.0' })
})

/* ===== Static files ===== */
app.use(express.static(WEBAPP_PATH, { maxAge: 0, etag: false }))
app.get('*', (req, res) => {
  const fp = path.join(WEBAPP_PATH, req.path === '/' ? 'index.html' : req.path)
  if (fs.existsSync(fp)) {
    ...
    res.sendFile(path.join(WEBAPP_PATH, 'index.html'))
  }
})
...
app.listen(PORT, '0.0.0.0', () => {
  console.log(`
╔═══════════════════════════════════════════╗
║     DiaMerna Persistent Server v1.0.0     ║
║     Port: ${PORT}                            ║
║     DB:   ${DB_PATH}  ║
